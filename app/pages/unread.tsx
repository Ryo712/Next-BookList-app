...
const ReadPage = () => {
    const [readTasks, setReadTasks] = useState<{ id: string; title: string; description: string; status: string; }[]>([]); // 読了したタスクの状態を管理

    useEffect(() => {
        const fetchReadTasks = async () => {
            try {
                const data = await getStatusOneData(); // getStatusOneData関数を使って読了したタスクを取得
                setReadTasks(data);
            } catch (error) {
